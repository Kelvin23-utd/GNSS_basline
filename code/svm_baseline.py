...
trials = [
    ...
    {"name": "rbf_gamma_0.01", "kernel": "rbf", "C": 10.0, "gamma": 0.01},
    {"name": "rbf_gamma_0.1", "kernel": "rbf", "C": 10.0, "gamma": 0.1},
]
...
# Print final result (required output)
print('5-fold CV accuracy: {:.1f}% ± {:.1f}%'.format(
    best_result["accuracy"] * 100,
    best_result["std"] * 100
))

# Confusion matrix (use one fold as example)
...
plt.tight_layout()
plt.savefig(os.path.join(FIG_DIR, "confusion_svm_finesat.png"), dpi=300, bbox_inches="tight")
plt.close()

# Save accuracy and F1 results to JSON
...
baseline_results["svm_finesat"] = {
    ...
    "cv_folds": 5,
    "preprocessing": "per-sample min-max normalization to [0,1]",
    "trials": results,
}
